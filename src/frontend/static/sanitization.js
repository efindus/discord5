...
const expressions = {
    ...
    built_in: /^(\s)*(ios_base|string|wstring|stringstream|istringstream|ostringstream|auto_ptr|deque|list|queue|stack|vector|map|set|pair|bitset|multiset|multimap|unordered_set|unordered_map|unordered_multiset|unordered_multimap|priority_queue|array|shared_ptr)$/g,
...
const highlightPart = (text) => {
    ...
    if (mode === 'comment') {
    ...
    } else if (text.match(expressions.multiline_comment) !== null) {
        ...
        const end = text.slice(match.length).match(expressions.string_end);
        ...
        } else if (match.match(expressions.built_in) !== null) {
